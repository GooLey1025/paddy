...
def plot_attribution(attr, seq=None, figsize=(20, 2), alphabet=['A', 'C', 'G', 'T']):
    """Plot attributions as a sequence logo.

    Parameters
    ----------
    attr: numpy.ndarray, shape=(length, alphabet_size)
        Attribution values.

    seq: str or None, optional
        The sequence (for reference). If None, inferred from attr. Default is None.

    figsize: tuple, optional
        Figure size. Default is (20, 2).
    ...
    if seq is None and attr.shape[1] == len(alphabet):
        seq = characters(attr, alphabet=alphabet, allow_N=True)

    # Plot each position
    for pos in range(attr.shape[0]):
        # Get heights (absolute values)
        heights = numpy.abs(attr[pos, :])
        # Argsort to stack from bottom to top
        order = numpy.argsort(heights)

        bottom = 0
        for nuc_idx in order:
            height = attr[pos, nuc_idx]  # Use signed height
            if abs(height) < 0.01:  # Skip very small values
                continue

            nuc = alphabet[nuc_idx]
            color = colors.get(nuc, 'black')

            # Draw letter
            fp = FontProperties(family='monospace', weight='bold')
            text_path = TextPath((pos, bottom), nuc, size=abs(height), prop=fp)
            patch = PathPatch(text_path, facecolor=color, edgecolor='none')
            ax.add_patch(patch)

            bottom += abs(height)

    ax.set_xlim(-0.5, attr.shape[0] - 0.5)
    ax.set_ylim(attr.min(), attr.max())
...
def plot_weights(weights, figsize=(20, 2)):
    """Plot filter weights as sequence logos.

    Parameters
    ----------
    weights: numpy.ndarray, shape=(motif_length, alphabet_size)
        Filter weights.
    ...
    # Use information content approach
    # Convert to probabilities
    probs = numpy.exp(weights) / numpy.exp(weights).sum(axis=1, keepdims=True)

    # Calculate information content
    max_entropy = numpy.log2(probs.shape[1])
    entropy = -numpy.sum(probs * numpy.log2(probs + 1e-10), axis=1)
    info_content = max_entropy - entropy

    # Scale heights by information content
    heights = probs * info_content[:, numpy.newaxis]

    return plot_attribution(heights, figsize=figsize)


def plot_marginalization_heatmap(y_delta, names=None, figsize=(10, 8), cmap='RdBu_r'):
    """Plot marginalization effects as a heatmap.

    Parameters
    ----------
    y_delta: numpy.ndarray, shape=(n_sequences, n_targets)
        Difference in predictions (y_after - y_before).

    names: list or None, optional
        Target names for y-axis labels. Default is None.
    ...
    vmax = max(abs(y_delta.min()), abs(y_delta.max()))
    im = ax.imshow(y_delta.T, aspect='auto', cmap=cmap, vmin=-vmax, vmax=vmax,
    ...
    plt.colorbar(im, ax=ax, label='Difference in Predictions')

    return fig, ax


def plot_ism(ism_scores, seq=None, figsize=(20, 4), alphabet=['A', 'C', 'G', 'T']):
    """Plot ISM (in-silico saturation mutagenesis) scores as a heatmap.

    Parameters
    ----------
    ism_scores: numpy.ndarray, shape=(length, alphabet_size)
        ISM attribution scores.

    seq: str or None, optional
        The original sequence. Default is None.

    figsize: tuple, optional
        Figure size. Default is (20, 4).
    ...
    vmax = max(abs(ism_scores.min()), abs(ism_scores.max()))
    im = ax.imshow(ism_scores.T, aspect='auto', cmap='RdBu_r', vmin=-vmax, vmax=vmax,
                   interpolation='nearest')
    ...
    if seq is not None:
        # Mark original nucleotides
        for pos, nuc in enumerate(seq):
            if nuc in alphabet:
                nuc_idx = alphabet.index(nuc)
                ax.add_patch(patches.Rectangle((pos-0.5, nuc_idx-0.5), 1, 1,
...
def plot_spacing_heatmap(spacing_results, spacings, figsize=(10, 6)):
    """Plot motif spacing analysis results as a heatmap.

    Parameters
    ----------
    spacing_results: numpy.ndarray, shape=(n_spacings, n_targets)
        Predictions for each spacing.

    spacings: list
        List of spacings tested.
    ...
    im = ax.imshow(spacing_results.T, aspect='auto',
...
def plot_tracks(tracks, figsize=(20, 4), names=None):
    """Plot multiple genomic tracks.

    Parameters
    ----------
    tracks: numpy.ndarray, shape=(n_tracks, length)
        Genomic tracks to plot.

    figsize: tuple, optional
        Figure size. Default is (20, 4).

    names: list or None, optional
        Track names. Default is None.
    ...
    for i, ax in enumerate(axes):
        ax.fill_between(range(tracks.shape[1]), tracks[i], alpha=0.5)
        ax.set_ylabel(names[i] if names is not None else f'Track {i+1}')
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
...
# Helper functions for plot_logo annotations
...
def place_new_box(box, box_list, n_tracks=4, show_extra=True):
    ...
    while overlap_exists:
        ...
        if steps_down_taken == n_tracks:  # beyond the n_tracks limit: make boxes smaller
            ...
            overlap_exists = any([check_box_overlap(box, box2)
...
def place_new_bar(box, box_list, y_step=None, n_tracks=4, show_extra=True):
    ...
    overlap_exists = any([check_box_overlap_bar(box, box2)
                         for box2 in box_list])
    steps_down_taken = 0
    while overlap_exists:
        steps_down_taken += 1
...
def plot_logo(X_attr, ax=None, color=None, annotations=None, start=None,
    ...
    Parameters
    ----------
    X_attr: numpy.ndarray, shape=(4, -1) or (seq_length, 4)
        ...
        NOTE: Accepts (seq_length, 4) shape, will transpose to (4, seq_length) if needed.
    ...
    if not HAS_LOGOMAKER:
        raise ImportError(
            "Must install logomaker before using plot_logo. Install with: pip install logomaker")

    if not HAS_PANDAS:
        raise ImportError(
            "Must install pandas before using plot_logo. Install with: pip install pandas")

    ###
    # Main glyph plotting code
    ###

    # Handle shape - convert (seq_length, 4) to (4, seq_length) if needed
    if X_attr.shape[0] > X_attr.shape[1]:
        X_attr = X_attr.T
    ...
    if annotations is not None:
        ...
        if len(annotations_) > 0:
            ...
            for i, (_, row) in enumerate(annotations_.iterrows()):
                ...
                text_box = ax.text(
                    motif_start, y_offset_labels, text, fontsize=labelsize)
                ax.get_figure().canvas.draw()
                bbox = text_box.get_window_extent()

                # ...shift box down if it overlaps with previously drawn boxes
                bbox_new, steps_down_taken = place_new_box(
                    bbox, label_text_boxes, n_tracks=n_tracks, show_extra=show_extra)
                bbox_new_transformed = bbox_new.transformed(
                ...
                text_box.set_position(
                    (bbox_new_transformed.x0, bbox_new_transformed.y0))
            ...
            for i, (_, row) in enumerate(annotations_.iterrows()):
                ...
                if bar_color == (0.7, 0.7, 0.7) or bar_color == (1, 1, 1, 0):
                    continue
                ...
                bar_box_new_transformed = bar_box_new.transformed(
                    ax.transData.inverted())
                bar[0].set_ydata([bar_box_new_transformed.y0,
                                 bar_box_new_transformed.y1])
...
def plot_categorical_scatter(X, colors=None, **kwargs):
    ...
    Parameters
    ----------
    X: numpy.ndarray, shape=(alphabet_len, length) or (length, alphabet_len)
    ...
    **kwargs: any additional arguments
    ...
    # Handle shape - convert (length, alphabet_len) to (alphabet_len, length) if needed
    ...
    for i in range(X.shape[0]):
        idxs = numpy.abs(X).argmax(axis=0) == i
        plt.scatter(pos[idxs], X.sum(axis=0)[idxs], c=[c[i]], **kwargs)


def plot_attributions(models, X, func=None, attribute_kwargs=None,
    ...
    By default, `integrated_gradients` is used to calculate attributions but any
    function can be used as long as it has the same signature.


    Parameters
    ----------
    models: Paddy SeqNN model or list of models
        One or a set of models to apply to the sequence/s.

    X: numpy.ndarray, shape=(-1, len(alphabet), -1) or (len(alphabet), -1)
        One or more sequences 

    func: func, optional
        The attribution function to use. Default is integrated_gradients.
        Must accept (model, X, **kwargs) and return attributions.
    ...
    # Import attribution function
    if func is None:
        from .attribution import integrated_gradients
        func = integrated_gradients

    # Handle model/sequence combinations
    if not isinstance(models, list):
        if X.ndim != 3:
            raise ValueError(
                "X must have 3 dimensions when only providing one model.")

        models = [models for _ in range(X.shape[0])]

    elif X.ndim == 2:
        X = numpy.tile(X[numpy.newaxis, :, :], (len(models), 1, 1))

    else:
        if len(models) != len(X):
            raise ValueError(
                "X must have first dimension equal to number of models.")
    ...
    X_attrs = numpy.concatenate(X_attrs, axis=0)
...
def plot_pwm(pwm, name=None, alphabet=['A', 'C', 'G', 'T'], eps=1e-7):
    ...
    Parameters
    ----------
    pwm: numpy.ndarray, shape=(len(alphabet), length)
    ...
    if not HAS_LOGOMAKER:
        raise ImportError(
            "Must install logomaker before using plot_pwm. Install with: pip install logomaker")
