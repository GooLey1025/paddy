...
class SeqDataset:
    ...
    def __init__(
        ...
        self.make_dataset(data_type=data_type)
    ...
    def generate_parser(self, raw: bool = False, data_type="2d_to_2d"):
        """Generate parser function for TFRecordDataset."""
        ...
        def parse_proto(example_protos):
            ...
            if not raw:
                ...
                if data_type == "2d_to_1d":
                    # For 2d_to_1d, targets are just a vector
                    targets = tf.reshape(targets, [self.num_targets])
                else:
                    # For 2d_to_2d, targets are a 2D array
                    targets = tf.reshape(
                        targets, [self.target_length, self.num_targets])

                targets = tf.cast(targets, tf.float32)
                if self.targets_slice is not None:
                    if data_type == "2d_to_1d":
                        targets = tf.gather(targets, self.targets_slice)
                    else:
    ...
    def make_dataset(self, cycle_length=4, data_type="2d_to_2d"):
        ...
        dataset = dataset.map(self.generate_parser(data_type=data_type))
    ...
    def compute_stats(self):
        ...
        for seq_raw, targets_raw in dataset:
            ...
            if self.num_targets is None:
                ...
                targets_nonzero = np.logical_or(targets_nonzero,
                                                (targets1
                                                 != 0).sum(axis=0) > 0)
...
def untransform_preds(preds, targets_df, unscale=False, unclip=True):
    ...
    # sqrt
    sqrt_mask = np.array(
        [ss.find("_sqrt") != -1 for ss in targets_df.sum_stat])
