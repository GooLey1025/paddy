#!/usr/bin/env python3
"""
Single batch TFRecord writer for SloRice regression (continuous slope label).

This script processes a single batch of SNP data and writes it to a TFRecord file.
It accepts base64 encoded gzipped batch data as a command line argument or a path
to a file containing that payload.

Features written per example:
- ref_sequence: uint8-encoded indices of DNA letters
- alt_sequence: uint8-encoded indices of DNA letters
- slope: float label for regression
- strand: int64 (0 for '+', 1 for '-')
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='Write a single TFRecord batch for SloRice regression (slope label)',
...
def write_batch_tfrecord(batch_data: List[Dict[str, Any]], output_file: str, verbose: bool = False) -> int:
    ...
    Args:
        batch_data: List of dictionaries containing 'ref_seq', 'alt_seq', 'slope', 'strand'
