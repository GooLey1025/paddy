...
def conv_dna(
    ...
    if residual:
        ...
        if norm_type == "batch-sync":
            current = tf.keras.layers.BatchNormalization(
                momentum=bn_momentum, synchronized=True)(current)
        elif norm_type == "batch":
            current = tf.keras.layers.BatchNormalization(
...
def unet_conv(
    ...
    if norm_type == "batch-sync":
        current1 = tf.keras.layers.BatchNormalization(
            momentum=bn_momentum, synchronized=True)(current1)
        current2 = tf.keras.layers.BatchNormalization(
            momentum=bn_momentum, synchronized=True)(current2)
    elif norm_type == "batch":
        current1 = tf.keras.layers.BatchNormalization(
...
def transformer(
    ...
    **kwargs,
    ...
    if dense_expansion == 0:
        ...
        final = transformer_dense(current, out_size, dense_expansion, l2_scale,
                                  dropout, kernel_initializer)
...
def transformer_split(
    ...
    **kwargs,
    ...
    if splits == 2:
        ...
        # transformer center
        current_center = tf.keras.layers.Reshape(
            (seq_len2, seq_depth))(current[:, 1:3, :, :])
...
def transformer_dense(inputs, out_size, dense_expansion, l2_scale, dropout,
                      kernel_initializer):
...
def dilated_residual_nac(inputs,
                         filters,
                         kernel_size=3,
                         rate_mult=2,
                         dropout=0,
...
def global_pool(
    inputs,
    pool_type="avg",
    axis=1,  # default axis=1, along seq_length/tracks dimension.
    **kwargs,
):
    """Apply global pooling to reduce a specified dimension.
    ...
    if axis == 2:
        # transpose to make feature dimension the middle dimension, for standard pooling.
        current = tf.transpose(current, perm=[0, 2, 1])

        if pool_type == "max":
            current = tf.keras.layers.GlobalMaxPooling1D()(current)
        elif pool_type == "attention":
            attention = tf.keras.layers.Dense(1, activation="tanh")(current)
            attention = tf.keras.layers.Softmax(axis=1)(attention)
            current = tf.keras.layers.Multiply()([current, attention])
            current = tf.keras.layers.Lambda(
                lambda x: tf.reduce_sum(x, axis=1))(current)
