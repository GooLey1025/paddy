...
def write_bedgraph(preds,
                   targets,
                   data_dir: str,
                   out_dir: str,
                   split_label: str,
                   bedgraph_indexes=None):
    ...
    with open("%s/statistics.yaml" % data_dir) as data_open:
        data_stats = yaml.safe_load(data_open)
        pool_width = data_stats["pool_width"]

    # read sequence positions
    seqs_df = pd.read_csv("%s/sequences.bed" % data_dir,
                          sep="\t",
