...
def load_igs_data(h5_path):
    ...
    Returns:
        Tuple of (igs_data dict, has_fold_enrichment bool)
    """
    ...
    # Check if fold_enrichment exists
    has_fold_enrichment = 'fold_enrichment' in h5_file
    if has_fold_enrichment:
        igs_data['fold_enrichment'] = h5_file['fold_enrichment'][:]
        print("  Found 'fold_enrichment' data")
    else:
        print("  WARNING: 'fold_enrichment' not found in H5 file")
        print("  This file cannot be used for correlation analysis")
    ...
    return igs_data, has_fold_enrichment
...
def main():
    ...
    # Load data
    igs_data, has_fold_enrichment = load_igs_data(args.h5)
    
    # Check if fold_enrichment is available for correlation analysis
    if not has_fold_enrichment:
        print("\nERROR: H5 file does not contain 'fold_enrichment' data")
        print("Cannot perform correlation analysis without fold enrichment values")
        sys.exit(1)
