...
for idx in $IDX_LIST; do
    gpu_id=$(( idx / 2 ))
    CUDA_VISIBLE_DEVICES=$gpu_id nohup paddy_ism_gene.py -o $PREFIX/${PREFIX}_${idx}_ism_gene.h5 --bath_size 4 --model_path $model_path $yaml NumChr.Rice_MSUv7.fa sub_gffs/P8_part_$(printf "%02d" $idx).gff > $PREFIX/${idx}_ism_gene.log 2>&1 &
done

IDX_LIST=$(seq 0 15)
for idx in $IDX_LIST; do
    gpu_id=$(( idx / 2 ))
    CUDA_VISIBLE_DEVICES=$gpu_id nohup paddy_ism_gene.py -o $PREFIX/${PREFIX}_$((idx + 16))_ism_gene.h5 --batch_size 4 --model_path $model_path $yaml NumChr.Rice_MSUv7.fa sub_gffs/P8_part_$(printf "%02d" $((idx + 16))).gff > $PREFIX/$((idx + 16))_ism_gene.log 2>&1 &
done
```
